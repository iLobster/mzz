...
{/strip}
<img src="{$SITE_PATH}/templates/images/jip.gif" class="jip" onmouseup="jipMenu.show(this, '{$jipMenuId}', [{foreach from=$jip item=jipItem name=jipItems}['{$jipItem.title}', '{$jipItem.url}', '{$jipItem.icon}', '{$jipItem.lang}']{if !$smarty.foreach.jipItems.last},{/if}{/foreach}], {ldelim}{foreach from=$langs item="langNames" key="langId" name="langs"}{$langId}: ['{$langNames[0]}', '{$langNames[1]}']{if !$smarty.foreach.langs.last},{/if}{/foreach}{rdelim});" alt="JIP Меню" />