...
<table border="0" width="100%" cellpadding="0" cellspacing="1">
    ...
    {foreach from=$users item=user}
        <tr>
            ...
            <td><a href="{url section=access action=deleteUserDefault params="$section/$class/$user_id"}" onClick="mzz_confirm('Вы хотите удалить этого пользователя?') &&  openWin('{url section=access action=deleteUserDefault params="$section/$class/$user_id"}', 'access_deleteuserdefault_{$section}_{$class}_{$user_id}', 500, 400); return false;"><img src="{$SITE_PATH}templates/images/delete.gif" width="16" height="16" /></a></td>
        </tr>
    {/foreach}
    {if $usersExists}
        <tr>
            <td align="center"><a href="{url section=access params="$section/$class" action=addUserDefault}" onClick="openWin('{url section=access params="$section/$class" action=addUserDefault}', 'access_adduserdefault_{$section}_{$class}', 500, 400); return false;"><img src="{$SITE_PATH}templates/images/add.gif" width="16" height="16" /></a></td>
...
<table border="0" width="100%" cellpadding="0" cellspacing="1">
     {foreach from=$groups item=group}
        <tr>
            ...
            <td><a href="{url section=access action=deleteGroupDefault params="$section/$class/$group_id"}" onClick="mzz_confirm('Вы хотите удалить эту группу?') && openWin('{url section=access action=deleteGroupDefault params="$section/$class/$group_id"}', 'access_deletegroupdefault_{$section}_{$class}_{$group_id}', 500, 400); return false;"><img src="{$SITE_PATH}templates/images/delete.gif" width="16" height="16" /></a></td>
        </tr>
    {/foreach}
    {if $groupsExists}
        <tr>
            <td align="center"><a href="{url section=access params="$section/$class" action=addGroupDefault}" onClick="openWin('{url section=access params="$section/$class" action=addGroupDefault}', 'access_addgroupdefault_{$section}_{$class}', 500, 400); return false;"><img src="{$SITE_PATH}templates/images/add.gif" width="16" height="16" /></a></td>
