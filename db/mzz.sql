...
INSERT INTO `page_page_lang` (`id`, `lang_id`, `title`, `content`, `keywords`, `description`) VALUES 
  (1,1,'Добро пожаловать!!','Добро пожаловать в mzz v.0.3<br />\nМы рады представить вам новую версию нашего замечательного (?!) фреймворка. Этот релиз примечателен тем, что за то долгое время, прошедшее с выпуска v.0.2.1 было переписано очень много старого, и местами рудиментарного, кода.<br />\nНовая версия была выпущена под лозунгом \"попытаемся написать как можно более стабильный код\". Именно поэтому в /branches были перенесены разработки (+ портирование) подавляющего большинства модулей.<br />\nВ этот релиз были перенесены и, насколько это возможно, доведены до состояния \"без багов\" самые необходимые из предыдущих модулей: новости, страницы, файловый менеджер. А также - ряд системных: пользователи, права доступа, ...<br />\nИз изменений хотелось бы особо выделить полностью переписанный с нуля ORM. Теперь он более гибкий и немного более быстрый. Также, теперь mzz обзавёлся двумя политиками обработки прав пользователей: acl_simple и acl_ext. Упрощённый оперирует классами объектов, расширенный - так же как и раньше, конкретными объектами.<br />\nБыла переработана документация. Дописаны новые главы. Устаревшие - переписаны или удалены.<br />\nДа, мы ещё далеки от совершенства. Но, мне кажется, уже сейчас мы достойны частички вашего внимания :-)<br /><br />\n\nСпасибо за внимание. С уважением, Курносов Иван (zerkms).<br /><br />\n\nПо всем вопросам касаемо mzz вы можете всё так же обращаться в наш <a href=\"http://mzz.ru/forum\">форум</a>, либо irc://mzz@irc.rusnet.ru либо ко мне лично по любому из доступных вам способов связи.','',''),
