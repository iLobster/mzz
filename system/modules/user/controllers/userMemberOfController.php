...
/**
 * userMemberOfController: контроллер дл€ метода memberOf модул€ user
 *
 * @package modules
 * @subpackage user
 * @version 0.1
...
class userMemberOfController extends simpleController
{
    public function getView()
    {
        ...
        // провер€ем что найден нужный пользователь
        if ($id != $user->getId()) {
        ...
        if ($this->request->getMethod() == 'POST') {
            ...
            $this->response->setTitle('ѕользователь -> ' . $user->getLogin() . ' -> список групп');
