...
/**
 * userEditController: контроллер дл€ метода edit модул€ user
 *
 * @package modules
 * @subpackage user
 * @version 0.2
...
class userEditController extends simpleController
{
    protected function getView()
    {
        ...
        $validator->add('required', 'user[login]', 'ќб€зательное дл€ заполнени€ поле');
        if (!$isEdit) {
            $validator->add('required', 'user[password]', 'ќб€зательное дл€ заполнени€ поле');
        }

        $validator->add('callback', 'user[login]', 'ѕользователь с таким логином же существует', array('checkUniqueUserLogin', $editedUser, $userMapper));
        ...
        if ($validator->validate()) {
            ...
            if (!$isEdit) {
                // добавим созданного пользовател€ в группы с флагом 'is_default'
