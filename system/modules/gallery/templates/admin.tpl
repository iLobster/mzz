    ...
    <table cellspacing="0" cellpadding="3" class="tableList">
        ...
        {foreach from=$album->getPhotos() item="photo"}
            <tr>
                ...
                <td style="width: 120px;text-align: center;"><a href="{url route="galleryPicAction" album=$album->getId() name=$gallery->getOwner()->getLogin() id=$photo->getId() action="view"}"><img src="{$SITE_PATH}/{$photo->getThumbnail()}" alt="{$photo->getName()}, {$photo->getFile()->getSize()|filesize}" title="{$photo->getName()}, {$photo->getFile()->getSize()|filesize}" /></a></td>
