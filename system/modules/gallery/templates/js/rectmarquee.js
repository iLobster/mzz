...
Marquee.prototype = {
	...
	initDrag: function(id, event) {
		...
		this.startDimensionScope = Position.cumulativeOffset(this.getElement());
	...
	setCoords: function(x, y, w, h) {
		...
		if (this.toolbar) {
		    var imgs = this.toolbar.select('img');
    		if (x + w + 5 + parseInt(this.toolbar.getStyle('width')) / 2 > element.offsetWidth) {
    		  imgs[0].hide();
    		  imgs[1].show();
    		  (this.toolbar.select('div')[0]).setStyle({'float': 'left'});
    		  this.toolbar.setStyle({left: this.startPosition[0] + x - parseInt(this.toolbar.getStyle('width')) - 5 + 'px', top: this.startPosition[1] + y  + 'px'});
    		} else {
    		  imgs[0].show();
    		  imgs[1].hide();
    		  (this.toolbar.select('div')[0]).setStyle({'float': 'right'});
    		  this.toolbar.setStyle({left: this.startPosition[0] + x + w + 5 + 'px', top: this.startPosition[1] + y  + 'px'});
    		}
