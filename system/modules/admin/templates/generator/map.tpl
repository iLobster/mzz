...
({{foreach from=$data.additional key="additionalKey" item="additionalValue" name="additionalForeach"}}      {{$additionalKey}}' => {{$additionalValue|@var_export:true}}'{{if !$smarty.foreach.additionalForeach.last}}, {{/if}}{{/foreach}}){{/if}}
