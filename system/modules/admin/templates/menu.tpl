                ...
                {if count($module.actions) > 1 || (count($module.actions) === 1 && !isset($module.actions.admin))}
                    ...
                    {foreach from=$module.actions item="action"}
                        {if $action->getName() != 'admin'}
                                ...
                                <li>
<a {if $action->getName() eq $current_action} class="active"{/if} href="{if $moduleName == 'admin'}{url route=adminSimpleActions action=$action->getName()}{else}{url route=withAnyParam module='admin' name=$moduleName action=$action->getName()}{/if}">
{icon sprite=$action->getIcon()}
{if $action->getTitle()}{$action->getTitle()|i18n:$moduleName}{else}{$module.info->getName()}{/if}</a></li>
                        {/if}
                    {/foreach}
                    </ol>
                {/if}
