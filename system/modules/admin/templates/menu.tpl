   ...
   {foreach from=$menu item="module" key="moduleName"}
       ...
       <tr>
           ...
           <td class="menuSection">
               ...
               <a href="{if $action == 'admin'}{url route=withAnyParam section='admin' name=$moduleName action=$action}{else}{url route=default2 section=$moduleName action=$action}{/if}">
               {if $moduleName eq $requested_module && $action eq $current_action}<strong>{/if}
