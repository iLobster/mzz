...
class simpleMapperForTreeTest extends unitTestCase
{
	...
	public function testInsertRoot()
	{
		...
		$this->mapper->setTree(777);		
		$this->mapper->insertRoot($do);
		
		$this->assertEqual($do->getTreeKey(), 9);

		$root = $this->mapper->getRoot();

		$this->assertEqual($root->getPath(false), 'rootFoo');
		$this->assertEqual($root->getTreeId(), 777);
	}
