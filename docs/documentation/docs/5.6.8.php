...
<<code>>PHP Error Exception. Thrown in file D:\server\sites\mzz\system\exceptions\errorDispatcher.php (Line: 49) with message: 
Warning in file D:\server\sites\mzz\system\template\mzzFileSmarty.php:48: SplFileObject::__construct(D:\server\sites\mzz\www/templates/commentsFolder/commentsFolder.list.tpl) [function.SplFileObject---construct]: failed to open stream: No such file or directory<</code>>
<p>Это потому, что мы не создали необходимые шаблоны. Откроем файл commentsFolderListView.php из каталога views и изменим имя шаблона на comments.list.tpl. Затем перейдём в каталог с шаблонами (www/templates) и создадим каталог comments. В только что созданном каталоге создадим файл comments.list.tpl. В этот файл запишем произвольный текст для проверки - например 'hello world'. И снова обновим страницу. Теперь нашему взору должна предстать новость, после которой появилась наша надпись hello world.</p>