<p>Для функционирования The Data Mapper Pattern также нужен файл map - в котором находится схема (описание структуры) данных. Файлы map располагаются в подкаталоге <code>maps</code> каталога модуля. В этом файле описаны имена полей, которые есть в таблице, которую отражает доменный объект, имена акцессоров и мутаторов, а также некоторая другая инфORMация. Структура данного файла выглядит так:</p>
<<code ini>>
[Имя поля]
...
Опция <code>accessor</code> определяет метод для получения значения свойства, <code>mutator</code> метод для загрузки (записи) значения свойства в доменный объект.</p>
...
owns = "related.id"
                ...
                В полученном согласно этой схеме доменном объекте значением в свойстве <code>related_id</code> (получаемое с помощью мутатора <code>getRelated</code>), будет являться инстанцией класса <code>related</code> с нужными данными.<br />
