<p>Во MZZ встроен механизм скаффолдинга, который позволяет генерировать готовый к использованию код для стандартных действией над объектом (CRUD - Create, Retrive, Update, Delete). Действия Create и Update чаще всего очень похожи, поэтому мы их объединили в одно Save действие.</p>
...
<p>Откроем файл шаблона <tt>modules/blog/templates/create.tpl</tt> и приведем нашу формочку в более приятный вид. Во MZZ есть генератор форм, который генерирует HTML-код для различных элементов форм, выставляет дефолтные значение и восстанавливает введеные пользоваталем данные в форме в случае ошибки заполнения формы и отображает ее:</p>
