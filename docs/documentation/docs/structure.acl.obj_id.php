...
<p>Бывают ситуации, когда необходимо зарегистрировать объект в ACL и дать ему некоторое значение <code>obj_id</code>, но создавать целую сущность ради одной записи в таблице - не оправданно. Тогда следует прибегнуть к механизму так называемых "фейковых" объектов. В терминологии mzz - это такие объекты, которые фактически не существуют в БД, однако зарегистрированы в ACL и имеют свои значения <code>obj_id</code>. Это реализуется с помощью метода <code>getObjectId()</code> <a href="structure.classes.html#structure.classes.toolkit">тулкита</a>. "Фейковые" объекты идентифицируются по имени. Пример получения <code>obj_id</code> для "фейкового" объекта:</p>
<<code php>>
