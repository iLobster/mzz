...
В UNIX-подобной операционной системе для извлечения содержимого архива в корень веб-сервера (например, htdocs) используется следующий способ:
<<code bash>>
tar -xvzf <имя архива>.tar.gz htdocs/
<</code>></p>
...
<p>Например, DocumentRoot в конфигурации веб-сервера Apache имеет значение <code>c:\www</code>, mzz распакован в <code>c:\www\sites\mzz</code>. Соответственно URL будет иметь примерно следующий вид: <code>http://localhost/sites/mzz/</code>. В таком случае <code>SITE_PATH</code> должен иметь значение <code>/sites/mzz</code>, в <code>www/.htaccess</code> изменятся некоторые директивы:
<<code apache>>
#....
