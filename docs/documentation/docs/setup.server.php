...
<p>После распаковки проверьте <a href="setup.configuration.html">настройки</a> в файле <code>www/configs/config.php</code> и, если требуется, измените их на ваши.</p>

<p>Если mzz установлен не в корень веб-сервера, то необходимо в <code>SITE_PATH</code> указать URL-путь.</p>

<p>Например, DocumentRoot в конфигурации веб-сервера Apache имеет значение <code>c:\www</code>, mzz установлен в <code>c:\www\sites\mzz</code>. Соответственно URL будет иметь примерно следующий вид: <code>http://localhost/sites/mzz/www/</code>. В таком случае <code>SITE_PATH</code> должен иметь значение <code>/sites/mzz/www</code>, кроме этого, в <code>www/.htaccess</code> изменятся некоторые директивы:
<<code apache>>
#...
