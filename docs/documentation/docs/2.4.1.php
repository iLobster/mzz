<p>Перед использованием и при создании нового проекта необходимо проверить файл с конфигурацией (<code>config.php</code> в папке проекта) и изменить некоторые опции, если потребуется.</p>
<p>Описание опций:</p>
...
<div class="options_desc">Включение/отключение debug-режима. Возможные варианты: <code>true</code> или <code>false</code>. Если указано 'true',
 ...
 <strong>!</strong>Используйте debug режим только в процессе разработки сайта, в готовых проектах это опция должна быть отключена (false),
 так как в текстах ошибок может содержаться конфиденциальная информация</div>
...
<div class="options_desc">Идентификатор записи в Базе Данных для неавторизированных пользователей. Изменение требуется при наличии
 базы данных в которой уже есть пользователь с идентификатором установленным по умолчанию. <strong>!</strong>Пользователь с идентификатором <code>MZZ_USER_GUEST_ID</code> должен существовать</div>
