<p>Паттерн проектирования MVC (Model-View-Controller) организует и разделяет приложение на три отдельные роли:</p>
<ul>
...
<p>Слой модели представлен в mzz ORM. О нём вы можете прочитать в <a href="/structure.orm.html">отдельной главе</a>.</p>
<p>Слой представления использует Smarty в качестве шаблонного движка. Инстанция смарти представлена свойством <code>$this->smarty</code> в <code>simpleController</code>. Более подробное описание работы со Smarty вы можете найти на <a href="http://smarty.php.net">официальном сайте</a>.</p>
<p>Контроллер, это слой, связывающий модель и представление. Именно в контроллер поступают данные запроса, на основе которых происходит получение нужных данных с помощью модели, и именно в контроллере эти данные с "сыром" виде передаются в шаблон. Контроллер обрабатывает какое-то определённое событие (экшн). Рассмотрим типичный контроллер mzz (отображение списка новостей):</p>
<!-- php code 1 -->
...
<p>Данные, которые метод <code>getView()</code> возвращает с помощью <code>return</code>, будут добавлены к текущему ответу клиенту.</p>